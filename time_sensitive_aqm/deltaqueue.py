...
from pr3d.de import ConditionalGammaMixtureEVM, ConditionalGaussianMM, ConditionalGammaEVM
...
class DeltaQueue(SimpleQueue):
    ...
    def calc_expected_success(self,
        df : pd.DataFrame,
    ):
        ...
        try:
            x = np.array([np.array([x, y],dtype=np.float64) for x, y in zip(df['queue_length'], df['longer_delay_prob'])])
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            # before poping the head of queue, Delta algorithm kicks in
